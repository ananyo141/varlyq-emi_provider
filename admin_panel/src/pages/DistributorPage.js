...
const navigate=useNavigate();
    useEffect(() => {
        if(user.userType!=='superDistributor')
        {
            navigate('/404');
        }
        console.log(user);
   ...
   async function fetchDistributors()
    {
        const resp= await Network.fetchUser(userId,'distributor',page,pageSize);
        ...
        <>
            ...
            <Container  maxWidth="xl">
                <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
                    ...
                    {user.userType==='superDistributor' && <Button variant="contained" startIcon={<Iconify icon="eva:plus-fill"/>} onClick={handleNewClick}>
                        New User
                    </Button>}
                </Stack>
                                    ...
                                    {users.map((user,index) => {
                                        return <TableRow hover key={index}>
                                            ...
                                            <TableCell align="right">
                                                <IconButton size="large" color="inherit" onClick={(event)=>{handleOpenMenu(event,user)}}>
            ...
            <Dialog open={newUserDialogOpen} onClose={handleNewUserDialogClose}>
                ...
                <CreateUserForm userType={'distributor'} onClose={handleNewUserDialogClose} onSuccess={onUserSuccess} parentId={userId}/>
            </Dialog>
            <Dialog open={editUserDialogOpen} onClose={handleEditUserDialogClose}>
                <DialogTitle>Edit</DialogTitle>
                <EditUserForm userType={'distributor'} onClose={handleEditUserDialogClose} onSuccess={onUserSuccess} parentId={userId} user={menuUser}/>
            </Dialog>
            <Dialog open={transferFormOpen} onClose={handleTransferUserDialogClose}>
                <DialogTitle>Transfer</DialogTitle>
                <TransferPointsForm parentId={userId} childUser={menuUser} onClose={handleTransferUserDialogClose} onSuccess={onUserSuccess}/>
            </Dialog>

            <Dialog open={withdrawalFormOpen} onClose={handleWithdrawFormClose}>
                <DialogTitle>Withdraw</DialogTitle>
                <WithdrawPointsForm parentId={userId} childUser={menuUser} onClose={handleWithdrawFormClose} onSuccess={onUserSuccess}/>
            </Dialog>
            <Popover
                open={Boolean(open)}
                anchorEl={open}
            ...
            >
                ...
                {
                 menuUser!==null ? menuUser.status===true? <MenuItem sx={{color: 'error.main'}} onClick={handleBlockClickEdit}>
