...
export default function WithdrawPointsForm({parentId,childUser,onClose,onSuccess})
{
    ...
    async function handleWithdrawClick() {
        ...
        if(points>childUser.balance)
        {
            snackbar.showMessage('Insufficient Balance');
            return;
        }
        setButtonLoad(true);
    var resp=await Network.withdrawPoints(childUser.id,parentId,description,points);
    ...
    {
        setUserLoad(true);
        const resp=await Network.fetchUserDetail(childUser.id);
        setUserLoad(false);
        setUser(resp['user']);
    }
                ...
                <Grid item xs={8}>
                    <Stack direction={"column"}>
                        <Box display="grid"
                            ...
                            <Label
                                color={'success'} >Available Balance - {childUser.balance}</Label>
                        </Box>
                        <Box display="grid"
                            ...
                            <TextField  type={"number"} sx={{minWidth:300}}
                                        ...
                                        value={points}
                                        onChange={(e)=> {
                                            if(e.target.value>=childUser.balance)
                                        {
                                            setPoints(childUser.balance);
                ...
                <Grid item xs={8}>
                    ...
                    <Box display="grid"
                        ...
                        <Stack direction={"row"} marginX={-5}>
                            ...
                            <LoadingButton  loading={buttonLoad}  size="large"   onClick={handleWithdrawClick}>
                                Withdraw
                            </LoadingButton>
                        </Stack>
